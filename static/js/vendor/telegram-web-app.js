...
(function () {
  ...
  function postEvent(eventType, callback, eventData) {
    ...
    if (window.__telegramPostEventDisabled) {
    ...
    if (window.TelegramWebviewProxy !== undefined) {
      try {
        var proxyPayload = JSON.stringify(eventData);
        if (typeof window.TelegramWebviewProxy.postEvent === 'function') {
          window.TelegramWebviewProxy.postEvent(eventType, proxyPayload);
          callback();
          return;
        }
        if (typeof window.TelegramWebviewProxy.post === 'function') {
          window.TelegramWebviewProxy.post(eventType, proxyPayload);
          callback();
          return;
        }
        throw new Error('No post handler on TelegramWebviewProxy');
      } catch (postError) {
        console.warn('[Telegram.WebView] postEvent fallback error', postError);
        if (postError && postError.message && postError.message.includes('Method not found')) {
          window.__telegramPostEventDisabled = true;
        }
        callback(postError);
        return;
      }
